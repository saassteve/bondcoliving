...
Deno.serve(async (req: Request) => {
  ...
  try {
    let query = supabase
      ...
      .select(`
        id, name, slug, price, currency, duration_days, duration_type,
    ...
    await logApiRequest(context.id, "/passes", req.method, params, 200, req.headers.get("x-forwarded-for") ?? undefined);

    return jsonResponse({ data: data ?? [], meta: { total: (data ?? []).length } });
  } catch (err) {
    await logApiRequest(context.id, "/passes", req.method, params, 500, req.headers.get("x-forwarded-for") ?? undefined);
