...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (type === "apartment") {
      let query = supabase
        ...
        .select(`
          ...
          check_in, check_out, total_amount, currency, payment_status,
          booking_status, nights, special_requests, created_at, updated_at,
          apartment:apartments(id, name, building_id)
        `, { count: "exact" })
    ...
    await logApiRequest(context.id, "/bookings", req.method, params, responseStatus, req.headers.get("x-forwarded-for") ?? undefined);

    return jsonResponse(result, responseStatus);
  } catch (err) {
    await logApiRequest(context.id, "/bookings", req.method, params, 500, req.headers.get("x-forwarded-for") ?? undefined);
