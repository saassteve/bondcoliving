/*
  # Add Guest Management System

  1. New Tables
    - `guest_stays` - Complete guest booking and stay information
    - Enhanced `apartment_availability` - Links availability to guest stays
    - `apartment_ical_feeds` - External calendar sync capabilities

  2. Database Functions
    - `create_guest_stay()` - Add guest stay and sync calendar
    - `update_guest_stay()` - Update guest information
    - `get_upcoming_stays()` - Get upcoming check-ins/check-outs
    - `get_calendar_with_guests()` - Calendar view with guest details
...
-- Add guest_stay_id column to apartment_availability if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create RLS policies for guest_stays (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'guest_stays' AND policyname = 'Admin users can manage guest stays'
  ) THEN
    CREATE POLICY "Admin users can manage guest stays"
      ON guest_stays
      FOR ALL
      TO authenticated
      USING (
...
-- Create RLS policies for apartment_ical_feeds (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'apartment_ical_feeds' AND policyname = 'Admin users can manage ical feeds'
  ) THEN
    CREATE POLICY "Admin users can manage ical feeds"
...
CREATE INDEX IF NOT EXISTS idx_apartment_ical_feeds_active ON apartment_ical_feeds(apartment_id) WHERE is_active = true;
