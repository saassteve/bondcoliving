/*
  # Fix Availability Check - Remove Nested DECLARE Block

  ## Problem
  The function still has a nested DECLARE block which might be causing issues.
  PostgreSQL doesn't handle nested DECLARE blocks well when the outer record
  variable hasn't been assigned.

  ## Solution
  Flatten the function by declaring all variables at the top level.
  This avoids any potential issues with nested scopes and unassigned records.
*/
...
COMMENT ON FUNCTION check_pass_availability(uuid, date) IS 
  'Checks if a coworking pass is available for booking on a specific date. Uses flattened variable declarations to avoid nested scope issues.';
