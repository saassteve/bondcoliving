                ...
                <table className="min-w-full divide-y divide-slate-700">
                  ...
                  <tbody className="bg-slate-800 divide-y divide-slate-700">
                    {recentBookings.map((booking) => {
                      ...
                      return (
                        <tr key={booking.id} className={booking.status === 'pending_payment' ? 'bg-yellow-900/10' : ''}>
                          ...
                          <td className="px-3 py-2 whitespace-nowrap">
                            <div className="text-sm font-medium text-slate-100">
                              €{booking.total_amount ? booking.total_amount.toFixed(0) : '0'}
                            </div>
                          </td>
                          <td className="px-3 py-2 whitespace-nowrap">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              booking.payment_status === 'paid' ? 'bg-green-900/50 text-green-300' :
                              booking.payment_status === 'pending' ? 'bg-yellow-900/50 text-yellow-300 animate-pulse' :
                          ...
                          <td className="px-3 py-2 whitespace-nowrap">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              booking.status === 'pending_payment' ? 'bg-yellow-900/50 text-yellow-300' :
                              ...
                              {booking.status.replace('_', ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                            </span>
                          </td>
                          <td className="px-3 py-2 whitespace-nowrap">
                            <div className="text-xs text-slate-400">{timeAgo}</div>
                            <div className="text-xs text-slate-500">{createdDate.toLocaleDateString()}</div>
                          </td>
                        </tr>
                      );
                    })}
