...
const BookingBar: React.FC<BookingBarProps> = ({ onSearch, isSticky = false }) => {
  ...
  const getMinCheckOutDate = () => {
    ...
    minCheckOutDate.setDate(checkInDate.getDate() + 14); // Minimum 14 days stay
  ...
  const handleCheckInChange = (date: Date | null) => {
    if (date) {
      ...
      if (searchParams.checkOut) {
        ...
        const minCheckOut = new Date(date.getTime() + (14 * 24 * 60 * 60 * 1000)); // 14 days in milliseconds
  ...
  return (
    <div className={`bg-[#1E1F1E]/95 backdrop-blur-sm shadow-2xl p-2 sm:p-3 flex flex-col md:grid md:grid-cols-2 lg:flex lg:flex-row items-stretch gap-1 sm:gap-2 w-full border border-[#C5C5B5]/20 ${
      ...
      https://ucarecdn.com/958a4400-0486-4ba2-8e75-484d692d7df9/foundersbond.png          onChange={(e) => handleInputChange('location', e.target.value)}
                className="w-full text-[#C5C5B5] bg-transparent border-none outline-none text-sm cursor-pointer appearance-none pr-6 focus:outline-none focus:ring-0 focus:border-none select-none"
                style={{ 
