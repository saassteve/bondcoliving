...
const BookingList: React.FC<BookingListProps> = ({
  ...
  const filteredBlockouts = (filter === 'all' || filter === 'ical_blockout')
    ? blockouts.sort((a, b) => new Date(a.check_in_date).getTime() - new Date(b.check_in_date).getTime())
    : [];

  const combinedItems = [...filteredBookings, ...filteredBlockouts].sort((a, b) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => onFilterChange('ical_blockout')}
                className={`px-3 py-1 text-sm rounded-full ${
                  filter === 'ical_blockout'
              ...
              {combinedItems.map((item) => {
                ...
                return (
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                    {isBlockout ? (
                      <span className="text-sm text-gray-500 italic">From iCal Feed</span>
                    ) : (
                      <>
                        <button
              ...
              {combinedItems.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-gray-500">
                    No {filter === 'ical_blockout' ? 'iCal blockouts' : 'bookings'} found
                  </td>
                </tr>
              )}
            </tbody>
